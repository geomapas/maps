...
async function saveShpToCloud(layer) {
  ...
  // Si la capa es colaborativa, el documento a actualizar es el de la propietaria original
  // (no el del usuario local), para que los cambios —incluida la geometría— se propaguen
  // a todos los colaboradores. Si el usuario local ES el propietario, es lo mismo de siempre.
  const targetUid = (layer._isCollab && layer._ownerUid) ? layer._ownerUid : user.uid;
  const docRef = db.doc(`artifacts/${appId}/users/${targetUid}/capas_vectoriales/${layer.id}`);
  ...
  try {
    ...
    // IMPORTANTE: merge:true — un set() sin merge sobrescribe el documento entero y borraría
    // el campo checklist_data (los "visitado" de todos los colaboradores) cada vez que se
    // edita la geometría de la capa.
    await docRef.set({
      id: layer.id,
      name: layer.name,
    ...
    layer._lastSyncedGeojson = serializedGeoJson;
    layer.synced = true;
...
async function sendShareRequest() {
  ...
  try {
    ...
    if (isCollab) {
      ...
      // Asegurar que el documento de la capa ya existe y está actualizado en la nube
      // (por si nunca se había guardado, p.ej. una capa recién importada)
      await saveShpToCloud(layer);
      ...
      // Volcar al documento compartido el checklist YA marcado antes de compartir (p.ej.
      // recintos ya visitados). Sin esto, los colaboradores nuevos no ven ningún "visitado"
      // hasta que alguien vuelva a tocar esa geometría después de compartir la capa.
      const existingChecklist = currentSnap.exists && currentSnap.data().checklist_data ? currentSnap.data().checklist_data : {};
      const localChecklist = {};
      const feats = [];
      const collectFeats = g => {
        if (!g) return;
        if (Array.isArray(g)) { g.forEach(collectFeats); return; }
        if (g.type === 'FeatureCollection') g.features?.forEach(f => feats.push(f));
        else if (g.type === 'Feature') feats.push(g);
      };
      collectFeats(layer.geojson);
      feats.forEach(f => {
        const gid = f.properties?.__gid;
        if (!gid) return;
        const saved = getChecklistData(layer.id, gid);
        if (saved) localChecklist[gid] = saved;
      });
      const mergedChecklist = { ...existingChecklist, ...localChecklist };

      await ownerDocRef.set({ collaborators: merged, ownerEmail: myEmail, checklist_data: mergedChecklist }, { merge: true });
      // Marcar la capa local como colaborativa de inmediato, sin esperar al round-trip
      // del listener — así el propio propietario ya puede marcar checklist correctamente
      // aunque no recargue ni reciba aún el snapshot "modified".
      layer._hasCollaborators = true;
      if (typeof setCollabActive === 'function') setCollabActive(layer.id);
    }
            ...
            if (!existing) {
              ...
              try {
                ...
                if (layer) {
                  ...
                  layer._lastSyncedGeojson = data.geojson;
                  setCollabActive(layerId);
                  // Añadir badge collab en la lista
                }
              } catch(e) { console.error('Error capa collab:', e); }
            } else if (data.geojson && data.geojson !== existing._lastSyncedGeojson) {
              // El propietario (u otro colaborador) ha modificado la geometría de la capa
              // (añadido o eliminado geometrías) — refrescar la capa local para reflejarlo.
              try {
                const geojson = JSON.parse(data.geojson);
                ...
                map.off('zoomend', existing.leafletLayer?._onZoom);
                ...
                if (refreshed) {
                  ...
                  refreshed._ownerUid = ownerUid;
                  refreshed._lastSyncedGeojson = data.geojson;
                  setCollabActive(layerId);
                  if (savedLabels && typeof restoreLayerLabels === 'function') restoreLayerLabels(refreshed, savedLabels);
                }
                updateCounter();
              } catch(e) { console.error('Error al refrescar capa collab:', e); }
            } else {
              existing._lastSyncedGeojson = data.geojson;
              if (data.name && existing.name !== data.name) existing.name = data.name;
            }
          });
...
// Vuelca el checklist_data de Firestore al localStorage local, indexado por __gid (identificador
// estable de cada geometría), y repinta cada geometría según su propio estado real de "visitado"
// (antes se repintaba TODA la capa con el color base, perdiendo la marca de visitado de todas
// las geometrías al llegar cualquier cambio de un colaborador).
function syncCollabChecklistToLocal(layerId, checklistData) {
  Object.entries(checklistData).forEach(([gid, data]) => {
    const key = `cl_${layerId}_${gid}`;
    try { localStorage.setItem(key, JSON.stringify(data)); } catch(_) {}
  });
  const layer = shpLayers.find(l => l.id === layerId);
  if (layer && typeof refreshLayerChecklistStyle === 'function') {
    refreshLayerChecklistStyle(layer);
  }
}
...
// Guardar checklist en Firestore cuando la capa es colaborativa (gid = __gid estable de la geometría)
async function saveCollabChecklist(layerId, gid, data) {
  const layer = shpLayers.find(l => l.id === layerId);
  if (!layer) return;
  ...
  try {
    await docRef.set({ checklist_data: { [gid]: data } }, { merge: true });
  } catch(e) {
    console.error('No se pudo guardar checklist colaborativo:', e);
    // Si esto falla, casi siempre es un problema de permisos de Firestore: las reglas de
    // seguridad deben permitir que un colaborador (uid distinto al propietario) escriba
    // en el documento del propietario. Lo mostramos explícitamente porque antes fallaba
    // en silencio y el cambio parecía "perderse" sin explicación.
    toast('No se pudo sincronizar el visitado con la capa compartida (revisa permisos)', 'err');
  }
}
