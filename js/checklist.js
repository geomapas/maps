...
// ════════════════════════════════════════════════════════
  ...
  document.addEventListener('click', function(e) {
    ...
    if (btn) {
      ...
      // El último segmento tras el "_" es el __gid de la geometría (identificador estable,
      // alfanumérico); todo lo anterior es el layerId (que puede contener "_" también).
      const keyParts = key.replace(/^cl_/, '').match(/^(.+)_([a-zA-Z0-9]+)$/);
      ...
      // key tiene formato: cl_${layerId}_${gid}  (gid = identificador estable de la geometría)
      if (keyParts) {
        updateFeatureVisitedStyle(keyParts[1], keyParts[2], visitado);
      }
      ...
      if (typeof shpLayers !== 'undefined' && typeof updateFeatureVisitedStyle !== 'undefined') {
        shpLayers.forEach(layer => {
          ...
          _allF.forEach(f => {
            ...
            if (saved?.visitado) {
              updateFeatureVisitedStyle(layer.id, gid, true);
            }
          });
        });
      }
...
(function() {
  ...
  function mergeIntoLayer(layerId, newGeojson) {
    ...
    if (_mergedLayer) {
      _mergedLayer._isCollab = _savedCollab.isCollab;
      _mergedLayer._ownerUid = _savedCollab.ownerUid;
      _mergedLayer._hasCollaborators = _savedCollab.hasCollaborators;
    }
