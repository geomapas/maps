...
document.addEventListener('DOMContentLoaded', function() {
  ...
  saveBtn.addEventListener('click', () => {
    ...
    // ── Guardar y aplicar filtros ──────────────────────────────
    if (typeof layerFilters !== 'undefined') {
      const newFilters = [0,1,2].map(i => {
        const field  = document.getElementById(`filter-field-${i}`)?.value || '';
        const rawVal = document.getElementById(`filter-values-${i}`)?.value || '';
        const values = rawVal.split(',').map(v => v.trim()).filter(Boolean);
        return { field, values };
      }).filter(f => f.field && f.values.length);
      layerFilters[_editLayer.id] = newFilters;
      applyLayerFilter(_editLayer);
      if (typeof isFirebaseActive === 'function' && isFirebaseActive()) {
        updateShpFiltersInCloud(_editLayer.id, newFilters);
      }
    }
  ...
  // ── Botón: generar nueva capa a partir de los filtros actuales ──
  const filterExtractBtn = document.getElementById('filter-extract-btn');
  filterExtractBtn?.addEventListener('click', () => {
    if (!_editLayer) return;
    const filtersNow = [0,1,2].map(i => {
      const field  = document.getElementById(`filter-field-${i}`)?.value || '';
      const rawVal = document.getElementById(`filter-values-${i}`)?.value || '';
      const values = rawVal.split(',').map(v => v.trim()).filter(Boolean);
      return { field, values };
    });
    if (typeof extractFilteredLayer === 'function') {
      extractFilteredLayer(_editLayer, filtersNow);
    }
  });
  ...
  window.openLayerEditModal = function(layer) {
    ...
    // Poblar filtros
    const fields = getLayerFields(layer);
    const savedFilters = typeof layerFilters !== 'undefined' ? (layerFilters[layer.id] || []) : [];
    [0,1,2].forEach(i => {
      const sel = document.getElementById(`filter-field-${i}`);
      const inp = document.getElementById(`filter-values-${i}`);
      if (!sel || !inp) return;
      sel.innerHTML = '<option value="">— Sin filtro —</option>' +
        fields.map(f => `<option value="${esc(f)}">${esc(f)}</option>`).join('');
      const saved = savedFilters[i];
      sel.value = saved?.field || '';
      inp.value = saved?.values?.join(', ') || '';
    });
