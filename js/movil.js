  ...
  shpUnsubscribe = colRef.onSnapshot((snapshot) => {
    snapshot.docChanges().forEach((change) => {
      ...
      if (change.type === "added") {
        ...
        if (layer) {
          // Si la geometría cambió (p.ej. se eliminó una geometría desde otro dispositivo o
          // por un colaborador), reconstruir la capa con los datos actualizados. Antes este
          // caso se ignoraba por completo y sólo se refrescaban color/etiquetas/checklist.
          if (data.geojson && data.geojson !== layer._lastSyncedGeojson) {
            try {
              const geojson = JSON.parse(data.geojson);
              const savedLabels = typeof layerLabels !== 'undefined' && layerLabels[data.id]
                ? { fields: [...layerLabels[data.id].fields], visible: layerLabels[data.id].visible }
                : null;
              ...
              map.removeLayer(layer.pinLayer);
              if (layer.leafletLayer._onZoom) map.off('zoomend', layer.leafletLayer._onZoom);
              const idx = shpLayers.findIndex(l => l.id === data.id);
              ...
              if (refreshed) {
                ...
                refreshed._ownerUid = savedCollab.ownerUid;
                refreshed._hasCollaborators = savedCollab.hasCollaborators;
                refreshed._lastSyncedGeojson = data.geojson;
                if (savedCollab.hasCollaborators) setCollabActive(data.id);
                if (savedLabels && typeof restoreLayerLabels === 'function') restoreLayerLabels(refreshed, savedLabels);
              }
              updateCounter();
            } catch(e) { console.error('Error al refrescar geometría sincronizada:', e); }
          } else if (layer.color !== data.color) {
            layer.color = data.color;
          ...
          if (data.collaborators && data.collaborators.length) {
            currentLayer._hasCollaborators = true;
            setCollabActive(data.id);
          }
        }
