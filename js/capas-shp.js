...
function getChecklistData(layerId, gid) {
  try { return JSON.parse(localStorage.getItem(`cl_${layerId}_${gid}`) || 'null'); }
  catch(e) { return null; }
}
...
// Asegura que cada feature tenga un identificador estable (__gid) que NO cambia aunque
// se reordenen o se eliminen otras geometrías de la capa. Esta es la clave real que se usa
// para guardar el checklist (visitado/comentario/etc.), en lugar de la posición en el array.
// Antes, al usar la posición como clave, borrar una geometría desplazaba el índice de todas
// las que iban detrás, haciendo que el estado "visitado" apareciera en un recinto distinto
// del que se había marcado — este es el motivo del bug reportado.
function ensureFeatureGids(layerId, features) {
  features.forEach((f, idx) => {
    f.properties = f.properties || {};
    if (f.properties.__gid) return;
    f.properties.__gid = genGid();
    // Migración de checklists guardados con el esquema antiguo (indexado por posición),
    // sólo aplica la primera vez que la capa se carga tras esta actualización.
    if (layerId) {
      const oldKey = `cl_${layerId}_${idx}`;
      const newKey = `cl_${layerId}_${f.properties.__gid}`;
      try {
...
// Recalcula el color de cada geometría de una capa según su estado de checklist actual.
// Se usa tras sincronizar cambios remotos (colaboración) para que TODAS las geometrías
// reflejen su estado real de "visitado", en vez de repintarlas todas con el color base.
function refreshLayerChecklistStyle(layer) {
  if (!layer) return;
  layer.polyLayer.eachLayer(sub => {
    const gid = sub.feature?.properties?.__gid;
    if (!gid) return;
    const saved = getChecklistData(layer.id, gid);
    const c = saved?.visitado ? VISITED_COLOR : layer.color;
  ...
  layer.pinLayer.eachLayer(m => {
    if (!m._gid) return;
    const saved = getChecklistData(layer.id, m._gid);
    const c = saved?.visitado ? VISITED_COLOR : layer.color;
    if (typeof m.setIcon === 'function') m.setIcon(buildPinIcon(c));
  });
}
...
function enrichGeojsonWithChecklist(layerId, geojson) {
  ...
  feats.forEach(f => {
    ...
    const saved = gid ? getChecklistData(layerId, gid) : null;
    ...
    delete f.properties.__gid; // campo interno, no debe exportarse
  });
  return clone;
}
...
// Se localiza por su identificador estable (__gid), no por posición.
function updateFeatureVisitedStyle(layerId, gid, visitado) {
  ...
  layer.polyLayer.eachLayer(sub => {
    if (sub.feature?.properties?.__gid !== gid) return;
    if (typeof sub.setStyle === 'function') {
  ...
  const f = allFeatures.find(x => x.properties?.__gid === gid);

  const pins = [];
  layer.pinLayer.eachLayer(m => pins.push(m));
  const pinMarker = pins.find(m => m._gid === gid);
  if (pinMarker && f) {
...
function buildPopupHtml(properties, layerId) {
  // El identificador estable de la geometría (__gid) viaja dentro de sus propiedades,
  // así que no hace falta pasarlo aparte ni depender de su posición en el array.
  const featureIdx = properties?.__gid;
  const hasChecklist = layerId != null && !!featureIdx;
  ...
  if (!properties) attrsHtml += '<i style="color:#aaa">Sin atributos</i>';
    ...
    if (k === '__gid') continue; // campo interno, no mostrar
...
function addShpLayer(geojson, name, cloudId = null, shouldSaveToCloud = false, shouldZoom = true, forceColor = null) {
  ...
  // Asegurar que cada feature tiene un identificador estable (__gid). Debe hacerse ANTES
  // de importar el checklist embebido y de construir los estilos, para que ambos usen
  // siempre la misma clave estable en vez de la posición en el array.
  ensureFeatureGids(id, allFeatures);
  ...
  const polyLayer = L.geoJSON(geojson, {
    ...
    onEachFeature: (f, l) => { l.bindPopup(() => buildPopupHtml(f.properties, id)); }
  });
  ...
  const leafletLayer = {
    ...
    setStyle(s) {
      ...
      polyLayer.eachLayer(sub => {
        const gid = sub.feature?.properties?.__gid;
        const saved = gid ? getChecklistData(id, gid) : null;
      ...
      allFeatures.forEach(f => {
        ...
        if (!c) return;
        const gid = f.properties?.__gid;
        const saved = getChecklistData(id, gid);
        const pc = saved?.visitado ? VISITED_COLOR : newColor;
        const marker = L.marker(c, { icon: buildPinIcon(pc) });
        marker._gid = gid;
        marker.feature = f; // necesario para que applyLayerFilter pueda leer sus propiedades
        marker.bindPopup(() => buildPopupHtml(f.properties, id)).addTo(pinLayer);
      });
      // Reaplicar filtro activo si lo hubiera, ya que el pinLayer se acaba de reconstruir
      if (typeof applyLayerFilter === 'function' && typeof layerFilters !== 'undefined' && layerFilters[id]?.length) {
        applyLayerFilter(obj);
      }
    }
  };
...
function addShpToUnifiedList(layer) {
  ...
  item.innerHTML = `
    ...
    <div class="item-actions">
      ...
      <div class="layer-filter-active" title="Capa con filtros activos" style="display:none;">
...
// ═══════════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════════
// SISTEMA DE FILTROS POR CAMPO
// ═══════════════════════════════════════════════════════════════
// layerFilters[layerId] = [ {field:'USO_SIGPAC', values:['PS','PR']}, ... ]
const layerFilters = {};
...
function applyLayerFilter(layer) {
  ...
  const icon    = item?.querySelector('.layer-filter-active');
...
// Persiste filtros en Firestore
async function updateShpFiltersInCloud(layerId, filters) {
  if (typeof isFirebaseActive !== 'function' || !isFirebaseActive()) return;
  const uid = auth?.currentUser?.uid;
  if (!uid) return;
  try {
    await db.doc(`artifacts/${appId}/users/${uid}/capas_vectoriales/${layerId}`)
      .update({ filters: JSON.stringify(filters) });
  } catch(_) {}
}

// Restaura filtros cuando la capa llega de Firestore
function restoreLayerFilters(layer, filtersData) {
  if (!filtersData) return;
  try {
    const parsed = typeof filtersData === 'string' ? JSON.parse(filtersData) : filtersData;
    if (Array.isArray(parsed)) {
      layerFilters[layer.id] = parsed;
      applyLayerFilter(layer);
    }
  } catch(_) {}
}

// Genera una nueva capa a partir de las geometrías que cumplen los filtros indicados
// filters: [{field, values}, ...] (mismo formato que layerFilters)
function extractFilteredLayer(layer, filters) {
  const active = (filters || []).filter(f => f.field && f.values.length);
  if (!active.length) { toast('Define al menos un filtro antes de generar la capa', 'err'); return; }
  ...
  if (!matched.length) { toast('Ningún resultado cumple los filtros actuales', 'err'); return; }

  const newGeojson = { type: 'FeatureCollection', features: matched.map(f => ({ ...f })) };
  const filterSummary = active.map(f => `${f.field}: ${f.values.join('/')}`).join(', ');
  const newName = `${layer.name} (filtro: ${filterSummary})`.slice(0, 80);

  addShpLayer(newGeojson, newName, null, true, true);
  toast(`Nueva capa creada con ${matched.length} geometría${matched.length > 1 ? 's' : ''}`, 'ok');
}
