...
function showQueryPopupCultivo(d, latlng) {
  ...
  // Campaña: campo confirmado "exp_ano" (normalizado a EXP_ANO por extractData). Se dejan
  // otros nombres como fallback por si el servicio cambia el campo en el futuro.
  const campana = raw('EXP_ANO') || raw('CAMPANA') || raw('CAMPAÑA') || raw('ANO_CAMPANA') ||
                  raw('ANIO_CAMPANA') || raw('EJERCICIO') || raw('ANIO') || raw('ANO') || null;

  qpIdEl.className = 'qp-id cultivo-id';
  qpIdEl.textContent = idStr;
  qpDetailEl.innerHTML = [
    campana ? `<div class="qp-row"><span class="qp-label">Campaña</span><span>${esc(campana)}</span></div>`   : '',
...
// que se abre, así que delegamos el evento en el contenedor en lugar de en cada input.
document.getElementById('search-fields').addEventListener('keydown', e => {
  if (e.key !== 'Enter' || !e.target.classList.contains('sf-input')) return;
  e.preventDefault();
  if (!searchMode) return;
