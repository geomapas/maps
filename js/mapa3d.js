...
(function () {
  ...
  function buildProjectLayers() {
    ...
    shpLayers.forEach(function(l) {
      ...
      var featuresColored = feats.map(function(f) {
        var gid = f.properties && f.properties.__gid;
        var saved = gid ? getChecklistData(l.id, gid) : null;
        var c = (saved && saved.visitado) ? VISITED_COLOR : l.color;
        return Object.assign({}, f, {
          properties: Object.assign({}, f.properties, {
            _ml_color: c, _ml_lid: l.id, _ml_fgid: gid
          })
        });
      });
  ...
  function setupClickPopup() {
    ...
    map3DInstance.on('click', function(e) {
      ...
      var fgid = f.properties && f.properties._ml_fgid;
      if (!lid || !fgid) return;
      ...
      // Props originales sin los campos internos _ml_*, localizadas por __gid (identificador estable)
      var allFeats = [];
      ...
      var origFeature = allFeats.find(function(x) { return x.properties && x.properties.__gid === fgid; });
      var origProps   = origFeature ? Object.assign({}, origFeature.properties) : {};

      var html = buildPopupHtml(origProps, lid);
